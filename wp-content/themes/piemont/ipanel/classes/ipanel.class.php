...
if (!class_exists('IPANEL')) {
    ...
    class IPANEL
    {
            ...
            if ((isset($_GET['page'])) && ('ipanel_' . $this->args['ID'] == $_GET['page'])) {
                ...
                wp_localize_script(
                    ...
                    array(
                        ...
                        'reset_confirm_message' => esc_html__("Are you sure you want to reset back to the defaults?", 'piemont'),
                        'confirm_message' => esc_html__("Are you sure?", 'piemont'),
        ...
        function saveOptions($data, $panel_id)
        {
            ...
            $update = $this->updateOption($panel_id, serialize($data));
            $output['status']  = $update === false ? 'error' : 'succeed';
            $output['message'] = $output['status'] === 'error' ? esc_html__('Error!', 'piemont') : esc_html__('Options Updated.', 'piemont');
            $output['test'] = serialize($data);
        ...
        function ajaxRequestsHandle()
        {
            ...
            if ($_nonce === false) {
                die(
                    json_encode(
                        array(
                        'status' => 'error',
                        'message' => esc_html__('Error!', 'piemont')
                        )
                    )
                );
            }
            if ($_POST['action2'] == 'save_settings') {
                ...
                if ($_POST['import_type'] == 'code') { // Check if import data is code
                    ...
                    die(
                        json_decode(
                            array(
                            'status' => 'error',
                            'message' => esc_html__("You've not specified any data.", 'piemont')
                            )
                        )
                    );
                }
                ...
                if ($add_option === true) { // Check if add data was successfuly
                    ...
                    $output['message'] = esc_html__('Data has been imported successfully.', 'piemont');
                } else { // and if not
                    $output['status']  = 'succeed';
                    $output['message'] = esc_html__('An error occured while importing data.', 'piemont');
                }
                ...
                $output['message'] = $delete === false ? esc_html__('Error!', 'piemont') : esc_html__('Options Reset.', 'piemont');
                $this->saveAllOptions();
                ...
                if (@unlink(urldecode($_POST['file_path']))) {
                    $output['status']  = 'succeed';
                    $output['message'] = esc_html__('The file permanently deleted.', 'piemont');
                } else {
                    $output['status']  = 'error';
                    $output['message'] = esc_html__('ERROR!', 'piemont');
                }
                echo json_encode($output);
                ...
                if (current_user_can('upload_files')) {
                    ...
                    if (array_key_exists('error', $movefile)) {
                        ...
                        if ($is_image) {
                            ...
                            $HTMLRESULTS .= '    '.esc_html__('File URL', 'piemont').': ' . '<code>' . $movefile['url'] . '</code><br/><br/>' . "\n";
                            $HTMLRESULTS .= '    '.esc_html__('File Path', 'piemont').': ' . '<code>' . $movefile['file'] . '</code><br/><br/>' . "\n";
        ...
        public function saveAllOptions()
        {
            ...
            return $this->updateOption($this->args['ID'], (serialize($options)));
        }
        ...
        public function getOption($id = '')
        {
            ...
            if(is_array($data)) {
              ...
              $option = maybe_unserialize(get_option($this->args['ID']));
            }
        ...
        public function downloadExport()
        {
            if (isset($_GET['action']) && $_GET['action'] == 'ipanel-download-export-file') {
                ...
                if ($data === false) {
                    wp_die(esc_html__('Error in download the file.', 'piemont'));
                }
        ...
        static function getGoogleFonts($sort = 'alpha')
        {
            ...
            if ( false === ( get_transient( 'theme_googleFonts'  ) ) || sizeof(get_transient( 'theme_googleFonts'  )) == 0 ) {
                ...
                if($google_fonts_data === false) {
                  ...
                  echo 'Google Fonts JSON file does not found.';
        ...
        public function menuPage()
        {
            ...
            $theme_key_var = 'piemont_license_key_status';
            ...
            if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'registration_complete') {
              update_option($theme_key_var, 'activated');
              delete_option('piemont_license_skip_date');
            }
            ...
            if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'registration_skip') {
              update_option('piemont_license_skip_date', strtotime("+1 day"));
              update_option($theme_key_var, 'activated');
            }

            if(strtotime("now") > get_option( 'piemont_license_skip_date', 0 ) && get_option( 'piemont_license_skip_date', 0 ) > 0) {
              delete_option('piemont_license_skip_date');
              delete_option($theme_key_var);
            }
            ...
            <div class="ipanel-form-stuff" style="margin-top: 20px;">
                ...
                <button class="button button-primary ipanel-save-settings"><?php echo esc_html__('Save All Changes', 'piemont'); ?></button>
                <h1 class="theme-cp-title"><?php echo esc_html($this->args['menu']['page_title']); ?><sup><?php $my_theme = wp_get_theme(); echo esc_html($my_theme->get( 'Version' ));?></sup></h1>
                <div class="buttons-panel-right">
                <a class="button button-primary ipanel-more-themes" target="_blank" href="http://magniumthemes.com/themes/?from=hdwp_cp"><?php echo esc_html__('More themes', 'piemont'); ?></a>
                <button class="button button-secondary ipanel-reset-settings"><i class="ipanel-black-icon trash"></i> <?php echo esc_html__('Reset', 'piemont'); ?></button>
                <?php if ($livePreview) {
                        echo '<button class="button button-secondary ipanel-toggle-preview"><i class="ipanel-black-icon eye"></i> '.esc_html__('Toggle Preview', 'piemont').'</button>';
                    }
                ?>
                </div>
                <div class="clr"></div>
                    ...
                    <tr>
                        <td class="ipanel-tabs-list" id="<?php echo esc_attr($this->args["ID"]); ?>-tabs-list">
                            ...
                            <!-- Left Side-->
                                <div id="ipanel-tabs">
                                    <ul>
                                        <?php
                                        if($theme_license_status !== 'activated'):
                                        ?>
                                        <li id="tab-registration"><a href="#" data-target="registration"><?php esc_html_e('Theme Registration', 'piemont'); ?></a></li>
                                        <?php endif; ?>

                                        <?php
                                        // Theme Options Framework show Tabs here. All data inside already correctly escaped and safe.
                                        echo piemont_wp_kses_data($this->generateTabs());
                                        ?>
                                    </ul>
                                </div>
                                <!-- Left Side-->
                        ...
                        <td id="ipanel-fields-container">
                            <div>
                                ...
                                <form id="theme-activation-form" action="themes.php?page=ipanel_PIEMONT_PANEL&act=activation" method="POST">
                                <div class="theme-activation-wrapper">
                                  <h1><?php esc_html_e('Welcome to Piemont!', 'piemont'); ?></h1>
                                  <p class="text-large"><?php echo wp_kses_post('Piemont is now installed and ready to use! Please register your purchase to get themes updates notifications,<br>import theme demos and get access to premium dedicated support. We hope you enjoy it!', 'piemont'); ?></p>
                                  <input name="email" type="email" class="activate-theme-email" placeholder="<?php esc_html_e('Enter your email', 'piemont'); ?>"/><br/><input name="code" type="text" placeholder="<?php esc_html_e('Enter your theme purchase code', 'piemont'); ?>" class="activate-theme-code"/>
                                  <a href="http://magniumthemes.com/go/how-to-find-purchase-code/" target="_blank" class="activate-theme-subblink"><?php esc_html_e('How to find your theme purchase code?', 'piemont'); ?></a><br/><input required class="button button-primary button-hero activate-theme-btn" type="button" value="<?php esc_html_e('Register theme', 'piemont'); ?>"/>
                                  <div class="theme-activation-message"></div>
                                  <p class="text-grey"><br><a href="themes.php?page=ipanel_PIEMONT_PANEL&act=registration_skip"><?php esc_html_e('I will register theme later (remind me tomorrow)', 'piemont'); ?></a><br><?php esc_html_e('By using this form you confirm that your purchase code, email (if you provided it) and site url will be saved on our servers for theme license verification purposes. If you provided email address (optional) you will receive important theme updates notifications and news (you can unsubscribe at any time).', 'piemont'); ?></p>
                                ...
                                <form id="ipanel-settings-form">
                                    ...
                                    echo piemont_wp_kses_data($this->generateFields($this->args['fields']));
                                    ?>
                                </form>
            ...
            <div class="ipanel-form-stuff">
                ...
                <button class="button button-primary ipanel-save-settings"><?php echo esc_html__('Save All Changes', 'piemont'); ?></button>
                <div class="theme-copyright">Theme developed by <a href="http://magniumthemes.com/?from=hdwp_cp" target="_blank">MagniumThemes Company</a>. Powered by <a href="http://magniumthemes.com/" target="_blank">Magnium Framework</a>.</div>
                <div class="buttons-panel-right">
                <a class="button button-primary ipanel-more-themes" target="_blank" href="http://magniumthemes.com/themes/"><?php echo esc_html__('More themes', 'piemont'); ?></a>
                <button class="button button-secondary ipanel-reset-settings"><i class="ipanel-black-icon trash"></i> <?php echo esc_html__('Reset', 'piemont'); ?></button>
                <?php if ($livePreview) {
                        echo '<button class="button button-secondary ipanel-toggle-preview"><i class="ipanel-black-icon eye"></i> '.esc_html__('Toggle Preview', 'piemont').'</button>';
                    }
        ...
        public function checkbox($options = null, $Repeater = array())
        {
            ...
            if (isset($options['repeater_item']) && $options['repeater_item'] === true) {
                $input = '    <input type="checkbox" class="switchery" name="' . $option_name . '" ' . $checked . ' />' . "\n";
            } else {
               // $input = '    <label>' . $checkbox_label . ' <input type="checkbox" class="switchery" name="' . $option_name . '" ' . $checked . ' /></label>' . "\n";
                $input = '<input type="checkbox" class="switchery" name="' . $option_name . '" ' . $checked . ' />' . "\n";
            }
            $input .= $this->conditionalLogic($options);
            if (isset($options['repeater_item']) && $options['repeater_item'] === true) {
        ...
        public function imageSelect($options = null, $Repeater = array())
        {
            if (isset($options['options'])) {
                ...
                foreach ($options['options'] as $option_key => $option_value) {
                    ...
                    $input .= '<img src="' . esc_url($option_value['image']) . '" title="' . esc_attr($option_value['label']) . '" alt="' . esc_attr($option_value['label']) . '" class="ipanel-image-select-image"/> ';
                    $input .= '<div class="tooltip-content">' . $option_value['label'] . '</div>';
                    $input .= '<input type="radio" value="' . esc_attr($option_key) . '" name="' . esc_attr($option_name) . '" ' . $checked . ' class="ipanel-image-field-radio"/>';
        ...
        public function quickUpload($options = null, $Repeater = array())
        {
            ...
            if (!empty($options['field_options']['btn_text'])) {
                ...
                $btn_text = esc_html__("Choose File", 'piemont');
            }
            ...
            if (filter_var($value['url'], FILTER_VALIDATE_URL) !== false) {
                ...
                if ($is_image) {
                    ...
                    $input .= '    '.esc_html__('File URL', 'piemont').': ' . '<code>' . $value['url'] . '</code><br/><br/>' . "\n";
                    $input .= '    '.esc_html__('File Path', 'piemont').': ' . '<code>' . $value['path'] . '</code><br/><br/>' . "\n";
                    $input .= '    <a href="#" class="ipanel-remove-qup-file" data-filePath="' . $value['path'] . '">'.esc_html__('Delete', 'piemont').'</a>' . "\n";
                    $input .= '    </div>' . "\n";
        ...
        public function repeater($options = null)
        {
            ...
            if (count($value)) { // If repeater has any item
                foreach ($value as $itemKey => $itemVal) {
                    ...
                    $inputs .= empty($options['field_options']['each_title']) ? '<h2 class="ipanel-repeater-title">' . esc_html__("Item", 'piemont') . '<span class="item_number"> ' . ($i + 1) . '</span>' . '</h2>' : '<h2 class="ipanel-repeater-title">' . sprintf($options['field_options']['each_title'], '<span class="item_number">' . $i + 1 . '</span>') . '</h2>';
                ...
                $inputs .= '<i class="ipanel-delete-icon ipanel-delete-repeater-item"></i>';
                $inputs .= '<i class="ipanel-drag-icon ipanel-drag-repeater-item"></i>';
                $inputs .= empty($options['field_options']['each_title']) ? '<h2 class="ipanel-repeater-title">' . __("Item", 'piemont') . ' 1' . '</h2>' : '<h2 class="ipanel-repeater-title">' . sprintf($options['field_options']['each_title'], '1') . '</h2>';
                $inputs .= '<div class="ipanel-repeater-fields">';
            ...
            $inputs .= '<button class="ipanel-button ipanel-button-white ipanel-repeater-new-item">'.esc_html__("New Item", 'piemont').'</button>';
        ...
        {
            $button_text  = isset($options['field_options']['button_text']) ? $options['field_options']['button_text'] : esc_html__("Upload", 'piemont');
            $media_title  = isset($options['field_options']['media_title']) ? $options['field_options']['media_title'] : esc_html__("Choose File", 'piemont');
            $media_button = isset($options['field_options']['media_insert_button_text']) ? $options['field_options']['media_insert_button_text'] : esc_html__("Choose File", 'piemont');
            if (isset($options['repeater_item']) && $options['repeater_item'] === true) {
                $option_name = $Repeater['id'] . '[' . $Repeater['pos'] . '][' . $options['id'] . ']';
            } else {
                $option_name = $options['id'];
        ...
        public function export($options = null)
        {
            ...
            if ($export_value === false) {
                $export_value = esc_html__("You need to save options first.", 'piemont');
            }
            $op = '';
            $op .= '<div class="ipanel-export-field">';
            $op .= '<textarea class="ipanel-export-text">' . $export_value . '</textarea><br/><br/>';
            $args = array(
            ...
            $op .= '<a href="' . esc_url($url) . '" class="ipanel-button ipanel-button-white ipanel-export-link-btn" target="_blank"> ' . esc_html__('Download Theme Options Backup File', 'piemont') . '</a>';
            $op .= '</div>';
        ...
        public function htmlpage($options = null)
        {
            ...
            return '<div class="ipanel-htmlpage" ' . $container_style . '>' . $options['name'] . '</div>';
        }
        ...
        public function import($options = null)
        {
            ...
            $op .= '<label><p>' . esc_html__('Enter Code', 'piemont') . '</p><textarea class="ipanel-import-text"></textarea></label>';
            $op .= '<br/><br/><label class="ipanel-import-file-label ipanel-button ipanel-button-white"> ' . esc_html__('Import File', 'piemont') . ' <input type="file" name="ipanel-import-file" class="ipanel-import-file"/></label>';
            $op .= '<span class="ipanel-chosen-filename"></span>';
            $op .= '<br/><br/><br/><a class="ipanel-button ipanel-button-white ipanel-import-button">'.esc_html__('Import', 'piemont').'</a>';
            $op .= '</div>';
            return $this->_fieldSection($options, $op);
        }
